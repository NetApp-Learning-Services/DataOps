...
class MyONNXMiniLM_L6_v2(EmbeddingFunction[Documents], Embeddings):
    ...
    # Added by Curtis
    def get_model(self):
        # __call__ will download the model if it doesn't exist
        return self._download_model_if_not_exists()

    # Added by Curtis
    # found:  https://github.com/langchain-ai/langchain/discussions/7818
    def embed_documents(self, documents: List[str]) -> List[List[float]]:
        encoded = [self.tokenizer.encode(d) for d in documents]
        response = [];
        for e in encoded:
            response.append(e.ids)
        return response
    ...
    def embed_query(self, query: str) -> List[float]:
        ...
        encoded = self.embed_documents([question])
        return encoded[0]
